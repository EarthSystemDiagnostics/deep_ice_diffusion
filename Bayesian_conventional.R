...
L <- 500 #No. of data points per section (window)
n <- floor(N/L) #For lowest resolution that uses all data points given L (non-overlapping windows)
section <- list()
...
#Applies conventional diffusion length estimate with Bayesian model over a running window covering the full Dome C record
for (i in 1:n){
  
  section[[i]] <- window(DomeC_ts, start = DomeC$depth[N - i * L + 1], end = DomeC$depth[N - (i - 1) * L]) #Starting from deepest section
  
  depth_est[i] <- time(section[[i]])[L/2]
  
  section_NAs <- sum(is.na(DomeC$d18O[(N - i * L + 1):(N - (i - 1) * L)])) #Starting from deepest section
...
write.csv(conv_data, "./conventional_sigmas.csv")
