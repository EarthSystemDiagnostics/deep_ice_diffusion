...
#Gaps in data
#MIS1: Depths 119.955 - 120.615, Ages 3.14848 - 3.17031
#          Depths 234.355 - 234.795, Ages 7.26902 - 7.28199
#          Depths 288.805 - 290.345, Ages 9.32127 - 9.37726
#MIS5: Depths 1583.505 - 1594.395, Ages 120.2833 - 121.1313
#      Depths 1638.505 - 1649.395, Ages 124.5008 - 125.3630
#      Depths 1693.505 - 1704.395, Ages 128.5310 - 129.2680

#Data from another script "./Bayesian_conventional.R", for Fig 5b
conv_data <- read.csv("./conventional_sigmas.csv")

#Data from another script: "./conventional_bias_simulated.R", for Fig 4a & 4b
bias_data <- read_csv('./conventional_bias.csv')
...
#Function that calculates the frequency limits for our fitting procedures (not used in final result)
...
find_P0_fit <- function(freq, spec, alpha_mu = 0.1, alpha_sigma = 1, beta_mu = 1.5, beta_sigma = 1, phi_scale = 1){
  ...
  fit <- P0_mod$sample(data = stan_data, chains = 4, parallel_chains = 4, refresh = 500, save_warmup = TRUE)
...
new_diffusion_length_estimate <- function(P0_age, P0_d18O, P_age = MIS19$age, P_depth = MIS19$depth, P_d18O = MIS19$d18O,
  ...
  P0_fit <- find_P0_fit(freq = P0$freq[P0_freq_inds], spec = P0$spec[P0_freq_inds], alpha_mu = P0_alpha_mu, alpha_sigma = P0_alpha_sigma, beta_mu = P0_beta_mu, beta_sigma = P0_beta_sigma)
  ...
  P0_fit_min <- P0_fit$summary()$q5[-(1:4)]
  P0_fit_max <- P0_fit$summary()$q95[-(1:4)]
  ...
  P0_params <- tibble(alpha_ests = as.vector(P0_fit$draws("alpha")),
                      beta_ests = as.vector(P0_fit$draws("beta")),
                      alpha_mu = P0_alpha_mu,
                      alpha_sigma = P0_alpha_sigma,
                      beta_mu = P0_beta_mu,
                      beta_sigma = P0_beta_sigma)
  ...
  return(list(P0_params = P0_params, params = params, P0_spec = P0_spec, MIS19_fit = MIS19_fit, P0_spec_full = P0_spec_full, P0_fit_summary = P0_fit, P_fit_summary = P_fit))
}
...
#Estimating freq range empirically (Not used in final result)
...
#Defining uninformative initial alpha and beta priors for P0 fits
P0_alpha_mu = 0.05
P0_alpha_sigma = 0.1
P0_beta_mu = 1.5
P0_beta_sigma = 1

#Defining weak diffusion length and noise priors
sigma_mu <- 0.4
sigma_sigma <- 0.4
noise_mu <- 0.07
noise_sigma <- 0.02
...
#Combining all parameter results
P0_params <- bind_rows(MIS1 = MIS1_results$P0_params, MIS5 = MIS5_results$P0_params, MIS9 = MIS9_results$P0_params, .id = "MIS")
P0_params$MIS <- factor(P0_params$MIS, levels = c('MIS1', 'MIS5', 'MIS9'))
P0_params_long <- P0_params %>% 
  select(alpha_ests, beta_ests, MIS) %>% 
  pivot_longer(., cols = c(alpha_ests, beta_ests), names_to = "parameter")

#Alpha estimates and P0 prior data frame
P0_prior_df_alpha <- P0_params %>%
  group_by(MIS) %>% 
...
#Beta estimates and P0 prior data frame
P0_prior_df_beta <- P0_params %>%
  group_by(MIS) %>% 
...
prior_df_sigma <- MIS_params %>% 
  ...
  summarise(
    ...
    x = seq(0.2, 0.45, length.out = 1000),
...
prior_df_noise <- MIS_params %>% 
  ...
  summarise(
    ...
    x = seq(0.04, 0.09, length.out = 1000),
...
P0_prior_df <- bind_rows(P0_prior_df_alpha, P0_prior_df_beta)
...
# Diffusion length estimate using the conventional method -----------------
...
fig_2a <- ggplot(data = MIS1_results$P0_spec) +
  ...
  geom_ribbon(aes(x = freq, ymin = fit_min, ymax = fit_max), colour = NA, fill = colfun[1], alpha = 0.3) +
...
fig_2b <- ggplot(data = MIS5_results$P0_spec) +
  ...
  geom_line(aes(x = freq, y = fit, colour = "Fit"), lwd = 1) +
  geom_ribbon(aes(x = freq, ymin = fit_min, ymax = fit_max), colour = NA, fill = colfun[2], alpha = 0.4) +
...
fig_2c <- ggplot(data = MIS9_results$P0_spec) +
  ...
  geom_line(data = fake, aes(x, y, colour = "MIS 1 fit"), lwd = 1) +
  geom_line(data = fake, aes(x, y, colour = "MIS 5 fit"), lwd = 1) +
  geom_ribbon(aes(x = freq, ymin = fit_min, ymax = fit_max), colour = NA, fill = colfun[3], alpha = 0.5) +
...
#This figure requires data set "bias_data" from another script: "conventional_bias_simulated.R"
...
#This figure requires data set "conv_data" from another script: "Bayesian conventional.R"

conv_data <- read_csv('./conventional_sigmas.csv')
...
fig_5b_main <- ggplot(data = conv_data) +
...
fig_5b_inset <- ggplot(data = conv_data) +
...
#Fig B1: P0 fit prior-posterior plots

pdf(file = "./fig_B1.pdf",
    width = 12,
    height = 4)

ggplot(P0_params_long) +
  geom_histogram(alpha = 0.5, aes(x = value, y = ..density.., fill = MIS), position = 'identity') +
  geom_line(data = P0_prior_df, aes(x = x, y = d, colour = 'prior')) +
  scale_fill_manual(values = colfun[1:3]) +
...
#Fig B2: Mean P fit prior-posterior plots

pdf(file = "./fig_B2.pdf",
    width = 12,
    height = 8)

ggplot(subset(MIS_params_long, MIS == 'mean')) +
  geom_histogram(alpha = 0.5, aes(x = value, y = ..density.., fill = 'posterior'), position = 'identity') +
  geom_line(data = subset(prior_df, MIS == 'mean'), aes(x = x, y = d, colour = 'prior')) +
  scale_fill_manual(values=colfun[4]) +
  scale_colour_manual(values=colfun[4]) +
  facet_wrap(~parameter, scales = "free") +
  theme_bw() +
  theme(legend.title=element_blank())
